...
def configure_logging():
    ...
    conf.rfHandler = logging.handlers.RotatingFileHandler(conf.logFilePath,
                                                          maxBytes=1000*1000,
                                                          backupCount=4)
...
class AttrDict(dict):
    '''helper class -- dictionary whose items can be accessed as attributes
    (ie foo['a'] == foo.a)'''
...
conf = AttrDict(
    ...
    rsyncopts=['-axuv', '--no-p', '--no-g', '--chmod=Dg+st,ugo+rx,Fo-w,+X']
    )
#             '--timeout=60',
#             '--exclude=.DS_Store',
#             '--exclude=Thumbs.db',
...
def parseTargetsFile(targetsFilePath=conf.targetsFilePath):
    '''Read our targets file for valid key/value pairs'''
    try:
            ...
            for linenum, line in enumerate(cfg):
                ...
                if len(target) != 2:
                    errmsg = "SyntaxError in targets file %s on line: %d\n" + \
                             "\t2 paths per line required, found %d" % \
                                     (targetsFilePath, linenum+1, len(target))
...
#                    raise ConfigFileTargetError(errmsg)
        ...
        errmsg = 'Syntax error in targets file %s on line %s' % \
                (targetsFilePath, linenum)
        raise FatalError(errmsg, e)
    ...
    if os.path.exists(pidFilePath):
        ...
        try:
            try:
                ...
                logger.warning(pidFilePath + \
                        ' existed, but no running PID.' + \
                        ' Attempting to cleaup expired PID file, and proceeding.')
...
def main():
    ...
    parser.add_option('-d', '--debug', action='store_true',
                      dest='debug', help='Turn on debugging output.')
    ...
    if options.silent:
        # don't intentionally output anything to console so cron will
        # never squack (unless there really is a problem)
        rsync_verbosity = '-q'
        logger.removeHandler(conf.consoleHandler)

    conf.rsyncopts.append(rsync_verbosity)
